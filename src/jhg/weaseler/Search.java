...
public class Search {
	...
	public String scoreReport(Article r) {
		String topLine = "Analyzing Article for :"+r.getFilename()+" .\n";
		int scoreVal = 0;
		int noSkill = 0;
		score.load();
		...
		if(contentWords.size()>0){
			...
			for(Weasel s:score.weasels){
				List<String> matchTokens = s.tokens;
				boolean found = false;
				for(String t:matchTokens){
					if(contentWords.contains(t)){
						foundLog.append("WeaselWord - "+s.name+" : '"+t+"',  adding score: "+s.score+"  ,tally: "+scoreVal+".\n");
						found = true;
						scoreVal += s.score;
						break;
					}
