...
public class BlueSolutionsCatalogDeploy {
	
	private static final String BLUE_SOLUTIONS = "blue_solutions.";
	...
	public static final String CONFIG = "bsc_table_config.txt";
	...
	public static String getFilename(String source, String dest){
		...
		return "BlueSolutionsCatalog" + U + ymd + U + source + "to" + dest +".txt";
	}
	...
	private static void transfer(boolean cleanTarget,Database source, Database target){
		...
		for(String sourceRelation:l){
			...
			if( Rdbms.SQLSERVER.equals(source.rdbms())){
				sourceTable = BLUE_SOLUTIONS+sourceRelation;
			}else{
				sourceTable = BLUE_SOLUTIONS+sourceRelation;
			}			
			String destTable = BLUE_SOLUTIONS+viewTableMapping.get(sourceRelation);
